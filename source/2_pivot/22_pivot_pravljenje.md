...
```{infonote}
Начин израчунавања у области Values можемо променити преко опције Value Field Settings. Поред подразумеваног збира (Sum), доступни су и Average (просек), Count (број уноса), Min и Max. Ова промена утиче само на приказ у пивот табели, не и на почетне податке.
```

### Корак 7: Додајте колоне (опционо)
Табелу у којој се види и на који начин су купци плаћали добили смо додавањем поља *начин плаћања* у зону Колоне (*Columns*)
...
```{infonote}
Графикон је повезан са пивот табелом, што значи да се свака промена у табели аутоматски приказује и на графикону.
```
